...
## Checking Process ID of Current Shell
```bash
echo $$
```
Displays PID of current shell
...
nohup command &
```
Runs a process that continues even after logout
...
time command
```
Shows how long a command takes to execute

---

## Checking Open Files by Process
```bash
lsof -p PID
```
...
## Checking CPU Usage per Process
```bash
...
Different ways to send signals:
